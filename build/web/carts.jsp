        ...
        <div id="search-nav" class="d-flex align-items-center">
            <div class="container text-center">
                <span class="body">
                    ...
                    <span class="txt-m-bold txt-orange default-cursor text-warning">Giỏ Hàng</span>
                </span>
            </div>
        </div>
        <!-- Cart Session -->
        <section class="mb-5" style="min-height: 250px">
            <div class="container">
                <c:choose>
                    ...
                    <c:otherwise>
                        <h3 class="text-black mb-3" style="font-size: 45px;">Tất cả sản phẩm</h3>
                        <table class="table text-center">
                            ...
                            <c:forEach items="${carts}" var="C">
                                <tag:productCart id="${C.value.product.id}" imgpath="${C.value.product.imgPath}" name="${C.value.product.name}" price="${C.value.product.price}" quantity="${C.value.quantity}"/>
                            </c:forEach>
                            </tbody>
                        </table>
                        <div class="row">
                            <h3 class="col-6 text-black">Total Amount: ${totalMoney}đ</h3>
                            <div class="col-6 text-end">
                                <c:url var="checkOut" value="CheckOutController"></c:url>
                                <a class="btn btn-success text-white btn-lg" href="${checkOut}">Thanh toán</a>
                            </div>
                        </div>
                    </c:otherwise>
                </c:choose>
