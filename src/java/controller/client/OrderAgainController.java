    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (OrderDetail orderDetail : listOrdersDetail) {
                subCarts.put(orderDetail.getProductId(), new Cart(productDAO.getProduct(orderDetail.getProductId()), orderDetail.getQuantity()));
            }
            ...
            if (carts != null && !carts.isEmpty()) { // Cart is not empty
                if (account != null) { // User had login
                    ...
                    if (result) {
                        ...
                        request.setAttribute("MSG_SUCCESS", "Đơn hàng của bạn thành công!");
                        request.getRequestDispatcher("UserViewOrderController").forward(request, response);
                    } else {
                        request.setAttribute("MSG_ERROR", "Những sản phẩm này đã hết hàng!");
                        request.getRequestDispatcher("UserViewOrderController").forward(request, response);
                    }
                } else {
