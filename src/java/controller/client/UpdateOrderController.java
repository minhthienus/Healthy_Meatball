            ...
            if (account != null) {
                ...
                if (orderStatusTxt != null && !orderStatusTxt.isEmpty()) {
                    ...
                    if (orderStatus == 1) {
                        ...
                        if (check) {
                            request.setAttribute("MSG_SUCCESS", "Hủy đơn hàng thành công!");
                            String url = (String) session.getAttribute("urlOrderHistory");
                            request.getRequestDispatcher(url).forward(request, response);
                        } else {
                            request.setAttribute("MSG_ERROR", "Hủy đơn hàng thất bại!");
                            String url = (String) session.getAttribute("urlOrderHistory");
                            request.getRequestDispatcher(url).forward(request, response);
                        }
                        ...
                        if (check) {
                            request.setAttribute("MSG_SUCCESS", "Đặt hàng thành công!");
                            String url = (String) session.getAttribute("urlOrderHistory");
                            request.getRequestDispatcher(url).forward(request, response);
                        } else {
                            request.setAttribute("MSG_ERROR", "Đặt hàng thất bại!");
                            String url = (String) session.getAttribute("urlOrderHistory");
                            request.getRequestDispatcher(url).forward(request, response);
                        }
                    }
