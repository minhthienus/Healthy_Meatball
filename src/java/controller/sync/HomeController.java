...
public class HomeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Our Product Session
            List<Product> listProducts = new ProductDAO().getTop4Products();
            request.setAttribute("listProducts", listProducts);
            
            // Best Seller Session
            Map<Integer, String> listCategories = new CategoryDAO().getCategories();
            List<Product> listBestSellerProducts = new ProductDAO().getTop4ProductsByCateId((int) listCategories.keySet().toArray()[0]);
            sessison.setAttribute("listCategories", listCategories);
            request.setAttribute("listBestSellerProducts", listBestSellerProducts);
