...
<html lang="en">
    ...
    <body class="animsition">
        ...
        <div id="search-nav" class="d-flex align-items-center">
            <div class="container text-center">
                <span class="body">
                    ...
                    <a class="home-page text-white text-decoration-none" href="CartController">Giỏ Hàng</a>
                    <span class="open-icon txt-m-bold text-white"><i class="bi bi-caret-right-fill"></i></span>
                    <span class="txt-m-bold txt-orange default-cursor text-warning">Thanh Toán</span>
                </span>
            </div>
        </div>
        <!-- Checkout Session -->
        <section class="mb-5" style="min-height: 250px">
            <div class="container">
                <h1 class="pb-4 default-cursor text-black" style="font-size: 50px;">Thanh Toán</h1>
                <div class="row">
                    <div class="col-lg-8 p-0 pb-3 pe-0 pe-lg-2 pb-lg-0">
                        <div class="rounded shadow p-3 bg-body">
                            <h3 class="default-cursor text-black mb-4">Tất cả sản phẩm</h3>
                            <table class="table text-center default-cursor">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                    <td>
                                        <c:url var="linkProduct" value="ProductDetailController">
                                            <c:param name="pid" value="${C.value.product.id}"></c:param>
                                        </c:url>
                                        <a href="${linkProduct}" class="text-decoration-none">${C.value.product.id}</a>
                                    </td><input type="hidden" name="productId" value="${C.value.product.id}">
                                    <td>
                                        <a href="${linkProduct}">
                                            <img src="${C.value.product.imgPath}" width="100">
                                        </a>
                                    </td>
                                    <td>${C.value.product.name}</td>
                                    <td>${C.value.product.price}đ</td>
                                    <td>${C.value.quantity}</td>
                                    <td>${C.value.product.price * C.value.quantity}đ</td>
                                    <c:set var="total" scope="request" value="${total + C.value.product.price * C.value.quantity}"/>
                                    </tr>
                                </c:forEach>
                                </tbody>
                            </table>
                            <h3 class="text-end default-cursor text-black">Tổng: <span style="color: red;">${not empty totalMoney ? totalMoney : total}đ</span></h3>
                        </div>
                    </div>
